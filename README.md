...
- `hybrid`または全6ショットを生成する`all_shots` Runway scope、360-credit上限、実秒数ベースの費用見積り
...
共通オプションは `--workspace`、`--config`、`--profile draft|preview|final`、`--provider local|runway`、`--runway-scope hybrid|all_shots`、`--narration-provider offline|openai`、`--agent-review-mode off|review`、`--compact` です。オプションはサブコマンドより前に置きます。
